...
export function createEksCluster(name: string) {
    ...
    let subnetIds;
    try {
        vpc = new awsx.ec2.Vpc(`vpc-${clusName}`, {
        ...
        clusterRole = new aws.iam.Role(`${clusName}`, {
        ...
        cluster = new eks.Cluster(clusName, {
        ...
        const subnetIds = aws.ec2.getSubnets(getSubnetsArgs).then(res => res.ids);
        subnetIds.then(ids => {
            ids.forEach(id => {
        ...
        // subnetIds = vpc.vpcId.apply(id => aws.ec2.getSubnetIds({ vpcId: id }));
        // subnetIds.apply(subnets => subnets.ids.forEach(id => {
        //     new aws.ec2.Tag(`cluster-tag-${id}`, {
        //         key: `kubernetes.io/cluster/${clusName}`,
        //         value: "shared",
        //         resourceId: id,
